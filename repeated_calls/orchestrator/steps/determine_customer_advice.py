...
class DetermineCustomerAdviceStep(KernelProcessStep):
    ...
    @kernel_function
    async def get_advice(self, state: State, kernel: Kernel, context: KernelProcessStepContext) -> None:
        """
        ...
        customer_clv = Customer.find_by_id(state.customer_id).clv
        if not customer_clv:
        ...
        # otherwise exit
        if customer_clv == "Low":
            print(f"Warning: Customer CLV value is low for customer ID {state.customer_id}")
        ...
        allDiscountsForProduct = Discount.find_by_product_id(state.cause_result.product_id)

        if not allDiscountsForProduct:
            print(f"Warning: No discount found for product ID {state.cause_result.product_id}")
            return
        ...
        if not matching_discount:
            print(
                f"Warning: No matching discount found for customer CLV value {customer_clv} and product ID {state.cause_result.product_id}"
            )
            return

        # Now we have a single discount object that matches the customer's CLV value
        print(
            f"Found matching discount: {matching_discount.percentage}% for customer ID {state.customer_id} with CLV {customer_clv} and duration {matching_discount.duration_months} months"
        )

        # Construct prompt
        prompt = RecommendationPrompt(state.cause_result, customer_clv, matching_discount)
        ...
        print("--- DetermineCustomerAdviceStep --- AI response ---:", response.content, sep="\n")
