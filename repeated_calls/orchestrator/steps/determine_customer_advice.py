...
from entities.database import Customer, CallEvent, Discount, HistoricCallEvent, Subscription
...
class DetermineCustomerAdviceStep(KernelProcessStep):
    ...
    def __init__(self):
        ...
        self._system_prompt = """
        Your job is to determine if the customer is eligible for a discount based on their customer lifetime value (CLV).
        To do this, you will be provided with a customer ID and a product ID, and you must identify the associated discounts.
        Write a message to the user with the customer ID, product ID, CLV value, discount percentage, and duration in months.
        If the customer is not eligible for a discount, write a message to the user indicating that.
    ...
    async def get_advice(self, cause_result, kernel: Kernel, context: KernelProcessStepContext) -> None:
        ...
        if not matching_discount:
            print(f"Warning: No matching discount found for customer CLV value {customer_clv_value} and product ID {cause_result.product_id}")
            return
            
        # Now we have a single discount object that matches the customer's CLV value
        print(f"Found matching discount: {matching_discount.percentage}% for customer ID {customer_id} with CLV {customer_clv_value} and duration {matching_discount.duration_months} months")
