...
class DetermineRepeatedCallerStep(KernelProcessStep):
    ...
    def __init__(self):
        super().__init__()
        self._system_prompt = "Your job is to provide the context for a customer. " \
    ...
    async def repeated_call(self, context: KernelProcessStepContext, kernel: Kernel, callstate) -> None:
        ...
        if hasattr(callstate, 'customer') and callstate.customer:
            ...
            user_message.append(f"Customer Since: {callstate.customer.relation_start_date.strftime('%Y-%m-%d')}")
            user_message.append("")
        ...
        if hasattr(callstate, 'call_event') and callstate.call_event:
            ...
            user_message.append(f"Timestamp: {callstate.call_event.time_stamp.strftime('%Y-%m-%d %H:%M:%S')}")
            user_message.append("")
        ...
        user_message.append("Based on this information, is the current call a repeated call about the same issue? Please analyze the timing between calls, similarity of issues discussed, and provide your reasoning.")
